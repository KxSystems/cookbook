 ...
 *
 * Author(s):   Sufian Al-Qasem & Attila Vrabecz
 *
 */
...
K connect(K host, K port, K appName, K conTimeout, K inactivityTime, K responseTimeout, K maxEventQueueSize, K consumerQueueSize)
{
    ...
    Q(blpapi_Session_start(Session),(blpapi_Session_destroy(Session),"blpapi_Session_start"))

    Q(blpapi_Session_openService(Session, "//blp/apiauth"), (blpapi_Session_destroy(Session), "openService apiauth"));
    Q(blpapi_Session_openService(Session, "//blp/mktdata"), (blpapi_Session_destroy(Session), "openService mktdata"));
    Q(blpapi_Session_openService(Session, "//blp/srcref"), (blpapi_Session_destroy(Session), "openService srcref"));

    Q(blpapi_Session_generateToken(Session, &c, 0), "blpapi_Session_generateToken");
    R 0;
}

K authorize(K token, K fakeSid)
{
    Q(token->t!=-KS||fakeSid->t!=-KI, "type")
    Q(!Session,"Session down")

    blpapi_Service_t *a;
    Q(blpapi_Session_getService(Session, &a, "//blp/apiauth"), "blpapi_Session_getService")

    Q(!(Identity = blpapi_Session_createIdentity(Session)), "blpapi_Session_createIdentity")

    blpapi_Request_t *r;
    Q(blpapi_Service_createAuthorizationRequest(a, &r, "AuthorizationRequest"), "blpapi_Service_createAuthorizationRequest")

    blpapi_Element_t *e;
    Q(!(e = blpapi_Request_elements(r)), "blpapi_Request_elements")

    blpapi_Element_t *t;
    Q(blpapi_Element_getElement(e, &t, "token", 0), "blpapi_Element_getElement")
    Q(blpapi_Element_setValueString(t, token->s, 0), "blpapi_Element_setValueString")

    blpapi_CorrelationId_t c;
    memset(&c, '\0', sizeof(c));
    c.size = sizeof(c);
    c.valueType = BLPAPI_CORRELATION_TYPE_INT;
    c.value.intValue = fakeSid->i;

    Q(blpapi_Session_sendAuthorizationRequest(Session, r, Identity, &c, 0, 0, 0), "blpapi_Session_sendAuthorizationRequest")
...
{
    Q(sym->t!=-KS||sid->t!=-KI||fields->t!=KS||resubscribe->t!=-KB,"type")
    Q(!Session,"Session down")
    ...
    Q(blpapi_SubscriptionList_add(s, sym->s, &c, (const char**)kS(fields), 0, fields->n, 0), "blpapi_SubscriptionList_add")

    ((int)resubscribe->g) ? (Q(blpapi_Session_resubscribe(Session, s, 0, 0),"blpapi_Session_resubscribe"))
    : (Q(blpapi_Session_subscribe(Session, s, Identity, 0, 0),"blpapi_Session_subscribe"));
...
K unsubscribe(K sid)
{
    ...
    Q(blpapi_Session_cancel(Session, &c, 1, 0, 0), "blpapi_Session_cancel")
    R 0;
}
